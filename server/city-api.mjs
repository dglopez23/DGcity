...
const COOKIE='cv_session', TTL=30*86400, MAX_BODY=240000;
...
export function validateCity(s){
 if(!s||![1,2,3].includes(s.format)||s.width!==(s.format===3?36:30)||s.height!==24)fail(400,'Formato de ciudad incompatible.');
 const width=s.width,total=width*s.height;
 ...
 const occupied=new Set();const buildings=s.buildings.map(b=>{if(!b||!Object.hasOwn(CAPS,b.type)||!number(b.level,1,CAPS[b.type],true)||!number(b.anchor,0,total-1,true)||!number(b.rot,0,3,true)||!number(b.palette,0,100,true)||!number(b.occ,0,b.type==='residential_block'?20000:800)||!number(b.fire,0,100))fail(400,'Edificio no válido.');const normal=b.type==='park'?(b.level>=5?[3,3]:[2,2]):['recycling','residential_block','university'].includes(b.type)?[2,2]:['coal','hospital','nuclear'].includes(b.type)?(b.rot%2?[1,2]:[2,1]):['solar','transit'].includes(b.type)?(b.rot%2?[2,1]:[1,2]):[1,1];const legacy=s.format===1||b.legacyFootprint===true||(s.format===2&&['university','transit','wastewater'].includes(b.type));const w=b.w??(((s.format===1&&['coal','hospital','nuclear','recycling'].includes(b.type))||(s.format<3&&['university','transit'].includes(b.type)))?1:normal[0]),h=b.h??(((s.format===1&&['coal','hospital','nuclear','recycling'].includes(b.type))||(s.format<3&&['university','transit'].includes(b.type)))?1:normal[1]);if(!number(w,1,3,true)||!number(h,1,3,true)||(!(w===normal[0]&&h===normal[1])&&!(legacy&&((['coal','hospital','nuclear','recycling','university','transit'].includes(b.type)&&w===1&&h===1)||(b.type==='park'&&w===2&&h===2)))))fail(400,'Dimensiones no válidas.');if(b.type==='residential_block'&&(!number(b.baseLevel,1,8,true)||b.level<4))fail(400,'Conjunto residencial no válido.');if(b.type==='wastewater'&&!legacy){const x=b.anchor%width;const adjacent=[b.anchor-width,b.anchor+width,...(x>0?[b.anchor-1]:[]),...(x<width-1?[b.anchor+1]:[])];if(!adjacent.some(i=>s.river[i]))fail(400,'La depuradora necesita un río adyacente.');}const x=b.anchor%width,z=Math.floor(b.anchor/width);if(x+w>width||z+h>24)fail(400,'Edificio fuera del mapa.');for(let dz=0;dz<h;dz++)for(let dx=0;dx<w;dx++){const i=b.anchor+dz*width+dx;if(occupied.has(i)||!s.landUnlocked[i]||(s.river[i]&&b.type!=='road'))fail(400,'Edificios solapados o terreno no edificable.');occupied.add(i);}return {type:b.type,level:b.level,w,h,baseLevel:b.baseLevel||4,legacyFootprint:legacy,anchor:b.anchor,rot:b.rot,palette:b.palette,occ:b.occ,fire:b.fire};});
