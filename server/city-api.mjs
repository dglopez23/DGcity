...
const COOKIE='cv_session', TTL=30*86400, MAX_BODY=180000;
const CAPS={road:8,residential:8,commercial:6,park:5,water:5,coal:5,school:5,clinic:4,library:4,police:5,recycling:5,wastewater:5,fire:5,solar:5,office:6,transit:4,university:4,hospital:4,nuclear:3,research:4,residential_block:8};
...
function cookie(token,age=TTL){return `${COOKIE}=${token}; Path=/api/ciudad-viva; HttpOnly; Secure; SameSite=Strict; Max-Age=${age}`;}
...
export function validateCity(s){
 if(!s||![1,2].includes(s.format)||s.width!==30||s.height!==24)fail(400,'Formato de ciudad incompatible.');
 ...
 for(const k of ['landUnlocked','river'])if(!Array.isArray(s[k])||s[k].length!==720||s[k].some(v=>typeof v!=='boolean'))fail(400,'Terreno no válido.');
 if(typeof s.debug!=='boolean'||!Array.isArray(s.buildings)||s.buildings.length>720)fail(400,'Edificios no válidos.');
 const occupied=new Set();const buildings=s.buildings.map(b=>{if(!b||!Object.hasOwn(CAPS,b.type)||!number(b.level,1,CAPS[b.type],true)||!number(b.anchor,0,719,true)||!number(b.rot,0,3,true)||!number(b.palette,0,100,true)||!number(b.occ,0,b.type==='residential_block'?20000:800)||!number(b.fire,0,100))fail(400,'Edificio no válido.');const normal=b.type==='park'?(b.level>=5?[3,3]:[2,2]):['recycling','residential_block'].includes(b.type)?[2,2]:['coal','hospital','nuclear'].includes(b.type)?(b.rot%2?[1,2]:[2,1]):b.type==='solar'?(b.rot%2?[2,1]:[1,2]):[1,1];const legacy=s.format===1||b.legacyFootprint===true;const w=b.w??(s.format===1&&['coal','hospital','nuclear','recycling'].includes(b.type)?1:normal[0]),h=b.h??(s.format===1&&['coal','hospital','nuclear','recycling'].includes(b.type)?1:normal[1]);if(!number(w,1,3,true)||!number(h,1,3,true)||(!(w===normal[0]&&h===normal[1])&&!(legacy&&((['coal','hospital','nuclear','recycling'].includes(b.type)&&w===1&&h===1)||(b.type==='park'&&w===2&&h===2)))))fail(400,'Dimensiones no válidas.');if(b.type==='residential_block'&&(!number(b.baseLevel,4,8,true)||b.level<4))fail(400,'Conjunto residencial no válido.');const x=b.anchor%30,z=Math.floor(b.anchor/30);if(x+w>30||z+h>24)fail(400,'Edificio fuera del mapa.');for(let dz=0;dz<h;dz++)for(let dx=0;dx<w;dx++){const i=b.anchor+dz*30+dx;if(occupied.has(i)||!s.landUnlocked[i]||(s.river[i]&&b.type!=='road'))fail(400,'Edificios solapados o terreno no edificable.');occupied.add(i);}return {type:b.type,level:b.level,w,h,baseLevel:b.baseLevel||4,legacyFootprint:legacy,anchor:b.anchor,rot:b.rot,palette:b.palette,occ:b.occ,fire:b.fire};});
 const researchPoints=s.researchPoints??0;if(!number(researchPoints,0,1e9))fail(400,'Investigación no válida.');
 const techTypes=['solar','university','hospital','nuclear'];
 const technologies=s.technologies??[...new Set(buildings.filter(b=>techTypes.includes(b.type)).map(b=>b.type))];
 if(!Array.isArray(technologies)||technologies.length>4||technologies.some(t=>!techTypes.includes(t))||new Set(technologies).size!==technologies.length)fail(400,'Tecnologías no válidas.');
 let loan=null;if(s.loan!=null){const l=s.loan;if(!number(l.principal,1,700000,true)||![3,6,12].includes(l.months))fail(400,'Préstamo no válido.');const interest=l.months*.02,total=l.principal*(1+interest),hourly=total/(l.months*30*24);if(!number(l.remaining,0,total))fail(400,'Deuda no válida.');loan={principal:l.principal,months:l.months,interest,total,remaining:l.remaining,hourly};}
...
export async function handleCityAPI(request,db){
 try{
  ...
  if(request.method!=='GET'&&request.headers.get('origin')!==ORIGIN)fail(403,'Solicitud de otro origen rechazada.');
  if(request.method==='POST'&&['register','login'].includes(route)){
   ...
   return json({user:{username:account.username}},200,{'Set-Cookie':cookie(token)});
  }
  ...
  if(route==='logout'&&request.method==='POST'){await db.prepare('DELETE FROM city_sessions WHERE token_hash=?').bind(user.token_hash).run();return json({ok:true},200,{'Set-Cookie':cookie('',0)});}
