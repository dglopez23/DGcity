...
const CAPS={townhall:8,road:8,residential:8,commercial:6,park:8,water:5,coal:5,school:5,clinic:4,library:4,police:5,recycling:7,wastewater:5,fire:5,solar:6,solar_complex:6,office:6,transit:4,university:4,hospital:4,nuclear:3,research:4,residential_block:8,heavy_industry:5,light_industry:5,industrial_complex:5,light_industrial_complex:5,prison:3,monument:1,wind:5,port:5,theater:5,opera:4};
...
function credentials(data){const username=typeof data.username==='string'?data.username.trim().normalize('NFKC'):'';if(!/^[\p{L}\p{N}_-]{3,24}$/u.test(username))fail(400,'Usa un nombre de 3–24 letras, números, guiones o guiones bajos.');if(typeof data.password!=='string'||data.password.length<8||data.password.length>128)fail(400,'El código de ciudad debe tener entre 8 y 128 caracteres.');return {username,key:username.toLocaleLowerCase('es'),password:data.password};}
export function validateCity(s){
 ...
 const occupied=new Set();const buildings=s.buildings.map(b=>{if(!b||!Object.hasOwn(CAPS,b.type)||!number(b.level,1,CAPS[b.type],true)||!number(b.anchor,0,total-1,true)||!number(b.rot,0,3,true)||!number(b.palette,0,100,true)||!number(b.occ,0,b.type==='residential_block'?22400:896)||!number(b.fire,0,100))fail(400,'Edificio no válido.');if(b.yardTree!==undefined&&(b.type!=='residential'||typeof b.yardTree!=='boolean'))fail(400,'Árbol residencial no válido.');const merged=b.type==='park'&&b.parkLevels!==undefined;if(merged&&(!Array.isArray(b.parkLevels)||b.parkLevels.length!==2||b.parkLevels.some(n=>!number(n,5,8,true))||b.level<Math.min(...b.parkLevels)||b.rot!==0||!((b.w===6&&b.h===3)||(b.w===3&&b.h===6))))fail(400,'Parque Central no válido.');const normal=b.type==='townhall'?((b.w===1&&b.h===2)||(b.w===2&&b.h===1)?[b.w,b.h]:[2,2]):merged?[b.w,b.h]:b.type==='port'?(b.rot%2?[3,1]:[1,3]):b.type==='park'?(b.level>=5?[3,3]:[2,2]):['recycling','residential_block','university','industrial_complex','solar_complex','prison','opera','nuclear'].includes(b.type)?[2,2]:['townhall','coal','hospital','light_industrial_complex'].includes(b.type)?(b.rot%2?[1,2]:[2,1]):['solar','transit','wastewater','heavy_industry'].includes(b.type)?(b.rot%2?[2,1]:[1,2]):[1,1];const oldNuclear=b.type==='nuclear'&&(((b.w??0)===2&&(b.h??0)===1)||((b.w??0)===1&&(b.h??0)===2));const legacy=(b.type==='wastewater'&&(b.w??1)===1&&(b.h??1)===1)||oldNuclear||s.format===1||b.legacyFootprint===true||(s.format===2&&['university','transit','wastewater'].includes(b.type));const w=b.w??((b.type==='wastewater'||(s.format===1&&['coal','hospital','nuclear','recycling'].includes(b.type))||(s.format<3&&['university','transit'].includes(b.type)))?1:normal[0]),h=b.h??((b.type==='wastewater'||(s.format===1&&['coal','hospital','nuclear','recycling'].includes(b.type))||(s.format<3&&['university','transit'].includes(b.type)))?1:normal[1]);if(!number(w,1,merged?6:3,true)||!number(h,1,merged?6:3,true)||(!(w===normal[0]&&h===normal[1])&&!(legacy&&((['coal','hospital','nuclear','recycling','university','transit','wastewater'].includes(b.type)&&w===1&&h===1)||(b.type==='nuclear'&&((w===2&&h===1)||(w===1&&h===2)))||(b.type==='park'&&w===2&&h===2)))))fail(400,'Dimensiones no válidas.');if(b.type==='residential_block'&&(!number(b.baseLevel,1,8,true)||b.level<4))fail(400,'Conjunto residencial no válido.');if(b.type==='wastewater'&&!legacy){const x=b.anchor%width;const adjacent=[b.anchor-width,b.anchor+width,...(x>0?[b.anchor-1]:[]),...(x<width-1?[b.anchor+1]:[])];for(let dz=0;dz<h;dz++)for(let dx=0;dx<w;dx++){const i=b.anchor+dz*width+dx,xx=i%width;adjacent.push(i-width,i+width,...(xx>0?[i-1]:[]),...(xx<width-1?[i+1]:[]));}if(!adjacent.some(i=>s.river[i]))fail(400,'La depuradora necesita un río adyacente.');}const x=b.anchor%width,z=Math.floor(b.anchor/width);if(b.type==='port'){const shore=Array.from({length:3},(_,k)=>b.rot===0?[x+1,z+k]:b.rot===1?[x+k,z-1]:b.rot===2?[x-1,z+k]:[x+k,z+1]);if(shore.filter(([xx,zz])=>xx>=0&&xx<width&&zz>=0&&zz<height&&s.river[zz*width+xx]).length<2)fail(400,'El puerto debe quedar paralelo al río, con agua en al menos dos posiciones de su lateral.');}if(x+w>width||z+h>height)fail(400,'Edificio fuera del mapa.');for(let dz=0;dz<h;dz++)for(let dx=0;dx<w;dx++){const i=b.anchor+dz*width+dx;if(occupied.has(i)||!s.landUnlocked[i]||(s.river[i]&&b.type!=='road'))fail(400,'Edificios solapados o terreno no edificable.');occupied.add(i);}return {type:b.type,...(b.type==='residential'?{yardTree:b.yardTree??false}:{}),level:b.level,w,h,...(merged?{parkLevels:b.parkLevels.slice()}:{}),baseLevel:b.baseLevel||4,legacyFootprint:legacy,anchor:b.anchor,rot:b.rot,palette:b.palette,occ:b.occ,fire:b.fire};});
 const rawInventory=s.inventory??[];
 if(!Array.isArray(rawInventory)||rawInventory.length>256)fail(400,'Inventario no válido.');
 // Reuse the building validator in an empty sandbox parcel; stored items occupy no city land.
 const inventory=rawInventory.map(b=>{
  if(!b||['road','townhall'].includes(b.type)||b.occ!==0||b.fire!==0||!number(b.anchor,0,total-1,true))fail(400,'Edificio de inventario no válido.');
  const river=Array(total).fill(false),anchor=width+1;
  if(b.type==='wastewater')river[width]=true;
  if(b.type==='port')for(let k=0;k<3;k++){const [x,z]=b.rot===0?[2,1+k]:b.rot===1?[1+k,0]:b.rot===2?[0,1+k]:[1+k,2];river[z*width+x]=true;}
  const checked=validateCity({...s,inventory:[],weather:undefined,municipalVersion:0,civicRoad:null,ordinance:null,ordinanceEverActivated:false,landUnlocked:Array(total).fill(true),river,buildings:[{...b,anchor}]}).buildings[0];
  return {...checked,anchor:b.anchor};
 });
 const weather=s.weather??{checkedDay:Math.floor(s.day/24),rainUntil:0,floodStart:0,floodUntil:0};
 if(!weather||typeof weather!=='object'||!number(weather.checkedDay,0,Math.floor(s.day/24),true)||!number(weather.rainUntil,0,s.day+24)||!number(weather.floodStart,0,s.day)||!number(weather.floodUntil,0,s.day+72)||(weather.floodUntil===0?weather.floodStart!==0:!number(weather.floodUntil-weather.floodStart,24-1e-7,72+1e-7)))fail(400,'Meteorología no válida.');
 const ordinances={housing:2,industry:2,local_trade:2,public_services:3,austerity:3,security:4,emergencies:4,circular:5,efficient:5,transit:6,business:6,technology:7,intensive:8,global:8};
 const ordinance=s.ordinance??null,ordinanceEverActivated=s.ordinanceEverActivated??!!ordinance,civicRoad=s.civicRoad??null,municipalVersion=s.municipalVersion??0;
 if((ordinance!==null&&(!Object.hasOwn(ordinances,ordinance)||s.cityLevel<ordinances[ordinance]))||typeof ordinanceEverActivated!=='boolean'||(ordinance&&!ordinanceEverActivated)||![0,1].includes(municipalVersion))fail(400,'Ordenanza municipal no válida.');
 const halls=buildings.filter(b=>b.type==='townhall');
 if(halls.length>1||halls.some(b=>b.level!==s.cityLevel||b.fire!==0))fail(400,'Ayuntamiento no válido.');
 if(civicRoad!==null&&(!number(civicRoad,0,total-1,true)||!buildings.some(b=>b.type==='road'&&b.anchor===civicRoad)))fail(400,'Carretera central no válida.');
 if(municipalVersion===1&&(halls.length!==1||civicRoad===null))fail(400,'Falta el Ayuntamiento o la carretera central.');
 if(municipalVersion===1){
  const roads=new Set(buildings.filter(b=>b.type==='road').map(b=>b.anchor)),seen=new Set([civicRoad]),queue=[civicRoad];
  const adjacent=i=>[...(i%width>0?[i-1]:[]),...(i%width<width-1?[i+1]:[]),i-width,i+width].filter(n=>n>=0&&n<total);
  for(let k=0;k<queue.length;k++)for(const n of adjacent(queue[k]))if(roads.has(n)&&!seen.has(n)){seen.add(n);queue.push(n);}
  if(seen.size!==roads.size)fail(400,'No se permiten carreteras aisladas.');
  const h=halls[0];let connected=false;for(let z=0;z<h.h;z++)for(let x=0;x<h.w;x++)if(adjacent(h.anchor+z*width+x).some(n=>roads.has(n)))connected=true;
  if(!connected)fail(400,'Ayuntamiento desconectado.');
 }
 const researchPoints=s.researchPoints??0;if(!number(researchPoints,0,1e9))fail(400,'Investigación no válida.');
 const techTypes=['solar','wastewater','university','hospital','nuclear','wind'];
 const technologies=s.technologies??[...new Set(buildings.filter(b=>techTypes.includes(b.type)).map(b=>b.type))];
 if(!Array.isArray(technologies)||technologies.length>techTypes.length||technologies.some(t=>!techTypes.includes(t))||new Set(technologies).size!==technologies.length)fail(400,'Tecnologías no válidas.');
 ...
 if(!number(tutorialStep,0,6,true)||!Array.isArray(adviceSeen)||adviceSeen.length>5||adviceSeen.some(t=>!['townhall','heavy_industry','research','light_industry','monument'].includes(t))||!Array.isArray(newsSeen)||newsSeen.length>1024||newsSeen.some(t=>typeof t!=='string'||t.length>300))fail(400,'Historial de consejos o noticias no válido.');
 return {inventory,weather:{checkedDay:weather.checkedDay,rainUntil:weather.rainUntil,floodStart:weather.floodStart,floodUntil:weather.floodUntil},ordinance,ordinanceEverActivated,civicRoad,municipalVersion,tutorialStep,adviceSeen:[...new Set(adviceSeen)],newsSeen:[...new Set(newsSeen)],cityName:cityName.trim(),dayNight:s.dayNight!==false,tutorialDone:s.tutorialDone??buildings.length>0,format:s.format>=3?s.format:2,width,height,gameOver:!!s.gameOver,mapSeed:s.mapSeed,money:s.money,day:s.day,tax:s.tax,cityLevel:s.cityLevel,diseaseDays:s.diseaseDays,crimeDays:s.crimeDays,alerts:s.alerts,debug:s.debug,landUnlocked:s.landUnlocked,river:s.river,buildings,unlockAt,researchPoints,technologies,loan};
}
...
export async function handleCityAPI(request,db,options={}){
 try{
  ...
  if(request.method==='POST'&&['register','login'].includes(route)){
   ...
   }else{const hash=await passwordHash(data.password,account?.salt||'missing-account-fixed-salt');if(!account||!equal(hash,account.password_hash))fail(401,'Nombre o código de ciudad incorrectos.');}
