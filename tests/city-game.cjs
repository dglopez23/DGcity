...
test('Municipal foundation is free, connected, permanent and evolves through eight bounded models',()=>{
 ...
 assert.equal(roads.length,6);assert.equal(hall.cells.length,2);assert.equal(hall.w,1);assert.equal(hall.h,2);assert.equal(s.money,15000);assert(g.operational(hall.anchor));assert.equal(g.municipalRoads().size,6);
 const models=[];for(let level=1;level<=8;level++){s.cityLevel=level;g.calc();assert.equal(hall.level,level);const parts=g.capture({...hall,level,rot:0});models.push(JSON.stringify(parts));for(const part of parts){assert(Math.abs(part.x)+part.sx/2<=.501);assert(Math.abs(part.z)+part.sz/2<=1.001);}}
...
test('Starter budget reaches the first promotion with only level-one unlocks',()=>{const s=g.newState();s.river.fill(false);g.setState(s);add(20,18,'road');add(16,16,'water');add(18,16,'commercial');add(19,16,'commercial');for(const x of [15,16,17,18,19])add(x,18,'residential');assert(s.money>=0);for(let n=0;n<2000&&s.cityLevel<2;n++)g.advance(.25);assert(s.cityLevel>=2,`stalled ${s.pop}/${s.happiness}`);assert(s.money>0);});
