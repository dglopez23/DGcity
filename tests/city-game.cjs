...
test('All static model components and moving industrial goods respect map layers',()=>{
 const s=reset();for(const layer of ['pollution','economy','satisfaction','education','health','security','fire','traffic']){s.layer=layer;for(const type of ['heavy_industry','light_industry','coal','water','school','clinic','recycling','monument']){const parts=g.captureLayer({type,level:3,palette:0});assert.equal(new Set(parts.map(p=>p.col)).size,1,`${layer} ${type}`);assert(parts.every(p=>p.em===0));}}
