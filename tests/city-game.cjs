...
test('Undo reverses construction, maximum upgrades, moving and demolition without rewinding city time',()=>{
 const s=reset();s.debug=false;add(7,7,'road');const before=s.money;g.build(g.idx(7,8),'residential');assert(g.undoLastAction());assert.equal(s.grid[g.idx(7,8)],null);assert.equal(s.money,before);assert(!g.undoLastAction());
 const house=add(7,8,'residential');house.occ=3;g.openPopup(house.anchor);const cash=s.money;g.upgradeToMaximum();assert(house.level>1);s.day+=.05;const day=s.day;assert(g.undoLastAction());assert.equal(house.level,1);assert.equal(s.money,cash);assert.equal(s.day,day);
 g.openPopup(house.anchor);g.moveSelected();g.placeMoved(g.idx(8,8));assert(g.undoLastAction());assert.equal(s.grid[g.idx(7,8)],house);assert.equal(s.grid[g.idx(8,8)],null);
 g.openPopup(house.anchor);g.demolishSelected();assert(g.undoLastAction());assert.equal(s.grid[g.idx(7,8)],house);assert.equal(house.occ,3);
 g.openPopup(house.anchor);g.upgradeSelected();const now=ctx.performance.now;ctx.performance.now=()=>12001;try{assert(!g.undoLastAction());}finally{ctx.performance.now=now;}
});
test('Inventory stacks equal levels, preserves variants and remains active after restoring',()=>{
 const s=reset();s.debug=true;const a=add(5,5,'residential'),b=add(6,5,'residential'),c=add(7,5,'residential',2);a.palette=0;b.palette=2;g.storeBuilding(a);g.storeBuilding(b);g.storeBuilding(c);assert.deepEqual(g.inventoryGroups().map(x=>x.count).join(','),'2,1');
 g.takeInventory(0);g.placeMoved(g.idx(8,5));assert.equal(s.tool,'inventory');assert.equal(s.inventory.length,2);assert.equal(s.grid[g.idx(8,5)].palette,0);assert(g.undoLastAction());assert.equal(s.inventory.length,3);assert.equal(s.tool,'inventory');assert.equal(s.grid[g.idx(8,5)],null);
});
test('Paused simulation cannot ignite or advance fires and idle rendering sleeps unless needed',()=>{
 const s=reset();s.debug=true;add(7,7,'road');const house=add(7,8,'residential');house.born=0;house.fire=.5;g.setSpeed(0);const random=ctx.Math.random;ctx.Math.random=()=>0;try{g.advance(.1);}finally{ctx.Math.random=random;}assert.equal(house.fire,.5);assert.equal(g.uniqueBuildings().filter(b=>b.fire).length,1);
 s.weather={checkedDay:4,rainUntil:0,floodStart:0,floodUntil:0};assert(!g.needsContinuousRender([house]));s.weather.rainUntil=s.day+2;assert(g.needsContinuousRender([house]));s.weather.rainUntil=0;house.born=999;assert(g.needsContinuousRender([house]));house.born=0;g.setSpeed(1);assert(g.needsContinuousRender([house]));g.setSpeed(0);
});
test('Static city geometry survives time, blackout and traffic updates but rebuilds for construction',()=>{
 const s=reset();s.debug=true;add(7,7,'road');const house=add(7,8,'residential');house.born=0;g.renderScenery(g.currentSun());const version=g.getCompiledVersion();s.day+=12;s.powerUse=100;s.powerCap=0;s.avgTraffic=70;s.diseaseDays=3;s.crimeDays=2;g.renderScenery(g.currentSun());assert.equal(g.getCompiledVersion(),version);
 g.build(g.idx(8,7),'road');g.renderScenery(g.currentSun());assert.equal(g.getCompiledVersion(),version+1);const key=g.sceneryGeometryKey(g.uniqueBuildings());s.landUnlocked[0]=false;assert.notEqual(g.sceneryGeometryKey(g.uniqueBuildings()),key);
});
test('Road caches retain exact components and invalidate on topology and congestion changes',()=>{
 const s=reset();s.debug=true;s.grid.fill(null);for(const [x,z] of [[5,5],[6,5],[7,5],[5,6],[6,6],[7,6]])add(x,z,'road');const first=g.roadComponents();assert.strictEqual(g.roadComponents(),first);const start=g.idx(5,5),target=g.idx(7,5),loads={};const direct=g.shortestRoadPath(start,new Set([target]),loads);assert(direct.includes(g.idx(6,5)));loads[g.idx(6,5)]=1e6;assert(!g.shortestRoadPath(start,new Set([target]),loads).includes(g.idx(6,5)));g.build(g.idx(8,5),'road');assert.notStrictEqual(g.roadComponents(),first);assert(g.roadComponents().comp[g.idx(8,5)]!=null);
});
test('Smoke and flames issue one draw call with per-vertex alpha',()=>{
 const s=reset();s.debug=true;add(5,5,'road');const plant=add(5,6,'coal',3);plant.fire=3;let calls=0;gl.drawArrays=()=>calls++;g.drawCityParticles(g.currentSun(),[plant]);delete gl.drawArrays;assert.equal(calls,1);
});
...
test('Display precision is capped and merged home upgrades follow four base residences',()=>{
 ...
 assert(g.CITY_GUIDE[2][2].includes('Balance por hora'));assert(g.CITY_GUIDE[2][2].includes('bancarrota'));
});
...
test('Extinguished fires always remove exactly one level, regardless of protection',()=>{
 const s=reset();s.debug=false;for(let x=8;x<=12;x++)add(x,7,'road');const b=add(8,8,'commercial',3);add(10,8,'fire',5);b.fire=.001;g.setSpeed(1);const random=ctx.Math.random;ctx.Math.random=()=>.999;try{g.advance(.01);}finally{ctx.Math.random=random;g.setSpeed(0);}assert.equal(b.level,2);assert.equal(b.fire,0);
});
